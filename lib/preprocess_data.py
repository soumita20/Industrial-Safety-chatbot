...
def prepare_ml_data1(text, flg_stemm=True, flg_lemm=True):
    corpus=[]
    for i,line in enumerate(text.Description_DL.tolist()):
      lst_stopwords = nltk.corpus.stopwords.words("english")
      
      # clean (convert to lowercase and remove punctuations and characters and then strip)
      text = re.sub(r'[^\w\s]', '', str(line).lower().strip())
...
def clean_DL_clean_data(text):
  corpus=[]
  for i,line in enumerate(text.Description_DL_clean.tolist()):
    #get rid of special characters
    text = remove_punctuations(line)
    text = clean_numbers(text)
